...
class PhilipsHueBulb(BaseBulb):
    ...
    def __check_error(self, json_response):
        desc_to_except = {
    ...
    def __request_with_error_check(self, **kwargs):
        response = requests.request(**kwargs)
        self.__check_error(response.json())
        return response.json()

    def _is_found(self):
        return self.name in self.__request_with_error_check(
